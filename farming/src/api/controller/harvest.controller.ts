...
import {
  ...
  GetHarvestDealQuery,
  getHarvestDealQueryDTO,
  GetHarvestDealResponse,
  getHarvestDealResponseDTO,
...
export class HarvestController {
  ...
  @GET({
    url: '/farming-cycles/:farmingCycleId/deals',
    options: {
      schema: {
        params: getFarmingCycleDetailParamDTO,
        querystring: getHarvestDealQueryDTO,
        response: {
          200: getHarvestDealResponseDTO,
        },
      },
      onRequest: verifyAccess,
    },
  ...
  async getDeals(
    request: FastifyRequest<{
      Params: GetFarmingCycleDetailParam;
      Querystring: GetHarvestDealQuery;
    }>,
  ): Promise<GetHarvestDealResponse> {
    const [deals, count] = await this.harvestService.getHarvestDeals({
      farmingCycleId: request.params.farmingCycleId,
      filter: request.query,
    });
