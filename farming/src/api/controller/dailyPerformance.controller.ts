  ...
  async get(
    ...
    return {
      ...
      data: dailyPerformances.map((dp) => {
        let age = 0;
        if (dp.farmingCycleStartDate && dp.closedDate) {
          age = differenceInCalendarDays(
            new Date(dp.closedDate.toString()),
            new Date(dp.farmingCycleStartDate.toString()),
          );
        } else if (dp.farmingCycleStartDate) {
          age = differenceInCalendarDays(new Date(), new Date(dp.farmingCycleStartDate.toString()));
        }
