...
import { ASSIGN_OTA_TOPIC } from '../libs/constants/mqttTopic';
import { RequestUser } from '../libs/types/index.d';
import { generateOrderQuery, replaceAll, topicToMac } from '../libs/utils/helpers';
...
import { SmartMonitorPublisher } from '../mqtt/publisher/smartMonitor.publisher';
...
export class DeviceSensorsService {
  ...
  @Inject(SmartMonitorPublisher)
  private smartMonitorPublisher: SmartMonitorPublisher;
  ...
  async getByMac(mac: string): Promise<IotDevice> {
      ...
      switch (device.deviceType) {
        ...
        default:
          await this.iotDeviceDAO.removeCache(
            `${CACHE_KEY_PREFIX.GET_ONE_IOT_DEVICE}:${input.mac.toLowerCase()}`,
          );
  ...
  async updateDeviceSensors(
    ...
    try {
      ...
      if (existingDevice.deviceType === 'SMART_MONITORING') {
        await this.iotDeviceDAO.removeCache(
          `${CACHE_KEY_PREFIX.GET_ONE_IOT_DEVICE}:${existingDevice.mac.toLowerCase()}`,
        );
        this.registerSmartMonitoring(existingDevice);
      }
  ...
  async deleteDeviceSensors(
    ...
    try {
      ...
      if (
        toBeDeleted.deviceType === 'SMART_MONITORING' ||
        toBeDeleted.deviceType === 'SMART_CAMERA'
      ) {
        await this.iotDeviceDAO.removeCache(
          `${CACHE_KEY_PREFIX.GET_ONE_IOT_DEVICE}:${toBeDeleted.mac.toLowerCase()}`,
        );
      }
  ...
  async triggerDeviceSensorAlert() {
    ...
    await Promise.allSettled(
      keys.map(async (redisKey: string) => {
  ...
  async registerSmartMonitoring(data: Partial<IotDevice>) {
    await this.smartMonitorPublisher.sendMapDeviceCommand({
      mac: data.mac || '',
    });

    await this.smartMonitorPublisher.sendMapSensorCommand({
      mac: data.mac || '',
      delayInSeconds: 3,
    });
  }
  ...
  async setDeviceAmmonia(data: SetDeviceAmmoniaSensorBody) {
    this.smartMonitorPublisher.sendSetAmmoniaCommand({
      mac: data.macAddress,
    });
  }
}
