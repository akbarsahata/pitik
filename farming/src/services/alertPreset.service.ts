...
import { ERR_ALERT_PRESET_CODE_EXIST, ERR_ALERT_PRESET_NOT_FOUND } from '../libs/constants/errors';
  ...
  async getMany(filter: GetAlertPresetQuery): Promise<[AlertPreset[], number]> {
    ...
    return this.alertPresetDAO.getMany({
      where: {
        ...
        alertPresetName: filter.alertPresetName ? ILike(`%${filter.alertPresetName}%`) : undefined,
