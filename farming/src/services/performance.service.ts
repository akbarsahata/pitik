...
export class PerformanceService {
  ...
  async getActualPerformance(farmingCycleId: string): Promise<PerformanceActual> {
    ...
    const date =
      differenceInCalendarDays(nowLocal, farmingCycle.farmingCycleStartDate) <= projectedDayNum
        ...
        : format(addDays(farmingCycle.farmingCycleStartDate, projectedDayNum), DATE_TIME_FORMAT);
    ...
    return {
      data: {
        ...
        mortality: {
          ...
          target: {
            min: tomoTargetToday?.minValue || 0,
            max: tomoTargetToday?.maxValue || 0,
          },
        },
        abw: {
          actual: latestDailyMonitoring?.bw || 0,
          target: {
            min: weightTargetToday?.minValue || 0,
            max: weightTargetToday?.maxValue || 0,
          },
        },
        feedConsumption: {
          actual:
          ...
          target: {
            min: kopaTargetToday?.minValue || 0,
            max: kopaTargetToday?.maxValue || 0,
          },
        },
        cycle: {
  ...
  async getCurrentFcrProjection(farmingCycleId: string): Promise<PerformanceProjectionTopGraph> {
    ...
    const feedConsumptionSumSack = await this.taskTicketDDAO.getLatestSum(
    ...
    const kopa = uomConverter(feedConsumptionSumSack, 'karung', 'gram') / estimatedPopulation;
    const kopaBenchmark =
      uomConverter(feedConsumptionTargetToday.minValue, 'karung', 'karung') / estimatedPopulation;
