...
import { constructAdditionalNotificationCoop, uomConverter } from '../../libs/utils/helpers';
...
export class GenerateAlertWorker extends BaseWorker<AlertJob> {
  ...
  protected async handle(
    ...
    try {
      ...
      await fcTriggers.reduce(async (prev, trigger) => {
        try {
          await prev;
  ...
  private async compareToDailyMonitoring(
    ...
    let dataToCompare = 0;
    const feedConsumptionGram =
      (data.feedIntake || 0) * (data.populationTotal || 0) -
      ((data.populationHarvested || 0) + (data.populationMortaled || 0));
    ...
    switch (variableCode) {
      ...
      case VAR_FEED_CON_CODE:
        ...
        dataToCompare = uomConverter(feedConsumptionGram, 'gram', 'karung');
        break;
      case VAR_ABW_CODE:
