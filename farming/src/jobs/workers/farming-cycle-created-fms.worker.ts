...
@Service()
export class FarmingCycleCreatedFmsWorker extends BaseWorker<FarmingCycle> {
  ...
  private taskFarmingCycleDDAO!: FarmingCycleTaskDDAO;
  ...
  protected workerName = QUEUE_FARMING_CYCLE_CREATED_FMS;
  ...
  protected async handle(
    ...
    const { autoGenerateAlerts, autoGenerateTasks } = await this.taskAlertDispatcher(data.id);

    const queryRunner = await this.taskFarmingCycleDDAO.startTransaction();
    ...
    try {
      ...
      const userCreator = { id: data.createdBy };
      ...
      if (autoGenerateTasks) {
        ...
        await this.taskFarmingCycleDDAO.createManyWithTx(
          farmingCycleTaskDPayloads,
          userCreator,
          queryRunner,
        );
      ...
      const fcChickStock: Partial<FarmingCycleChickStockD> = {
        farmingCycleId: data.id,
        userId: userCreator.id,
        transactionDate: data.farmingCycleStartDate,
        operator: '+',
        qty: data.initialPopulation,
        notes: 'Initial Population',
      };
      await this.farmingCycleChickStockDDAO.createOneWithTx(fcChickStock, userCreator, queryRunner);

      await this.taskFarmingCycleDDAO.commitTransaction(queryRunner);
    } catch (error) {
      await this.taskFarmingCycleDDAO.rollbackTransaction(queryRunner);
  ...
  private async taskAlertDispatcher(farmingCycleId: string) {
    const [[, countFcAlerts], [, countFcTasks]] = await Promise.all([
      ...
      await this.taskFarmingCycleDDAO.getMany({ where: { farmingCycleId } }),
    ]);
