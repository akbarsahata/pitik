...
export class CoopDAO extends BaseSQLDAO<Coop> {
  ...
  async getAvailableCoop(params: GetAvailableCoopQuery): Promise<[Coop[], number]> {
    ...
    if (params.farmCategory) {
      sql
        .where('farm.category = :farmCategory', { farmCategory: params.farmCategory })
        .andWhere(
  ...
  async closeFarmingCycleWithTx(id: string, queryRunner: QueryRunner) {
    ...
    await this.repository
      ...
      .set({
        ...
        purchaseRequestSapronakId: null,
  ...
  async updateManyWithTx(
    ...
    const updatedCoop = await queryRunner.manager.find(Coop, {
      where: {
        ...((params.id && {
          id: params.id,
        }) ||
  ...
  // eslint-disable-next-line class-methods-use-this
  async deleteOneWithTx(params: FindOptionsWhere<Coop>, queryRunner: QueryRunner): Promise<Coop> {
    const toBeDeleted = await queryRunner.manager.findOneOrFail(Coop, { where: params });

    await queryRunner.manager.delete(Coop, {
      id: toBeDeleted.id,
    });
