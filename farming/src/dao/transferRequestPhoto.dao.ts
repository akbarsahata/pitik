...
export class TransferRequestPhotoDAO extends BaseSQLDAO<TransferRequestPhoto> {
  ...
  async deleteManyWithTx(
    where: FindOptionsWhere<TransferRequestPhoto>,
    queryRunner: QueryRunner,
  ): Promise<TransferRequestPhoto[]> {
    const toBeDeleted = await queryRunner.manager.find(TransferRequestPhoto, { where });

    await queryRunner.manager.delete(TransferRequestPhoto, where);
