...
export class TaskTicketDAO extends BaseSQLDAO<TaskTicket> {
    ...
    const [result, count] = await Promise.all([
      this.repository
        ...
        .select([
          ...
          '0 as "potentialPoints"',
          'CASE WHEN fctd.task_code = \'TD-033\' THEN true ELSE false END as "isDailyReport"',
        ])
        .leftJoin(FarmingCycleTaskD, 'fctd', 'tt.ref_farmingcycletask_id = fctd.id')
  ...
  async getOneDetailById(id: string): Promise<TaskDetail> {
    const [taskTicket, taskTicketDetails] = await Promise.all([
      this.repository
        ...
        .select([
          ...
          'fctd.ref_farmingcycle_id as "farmingCycleId"',
          'CASE WHEN fctd.task_code = \'TD-033\' THEN true ELSE false END as "isDailyReport"',
        ])
        ...
        .where('tt.id = :id')
        .groupBy('tt.id')
        .addGroupBy('fctd.task_code')
        .addGroupBy('fctd.ref_farmingcycle_id')
        .setParameter('id', id)
        .getRawOne(),
    ...
    return {
      ...
      isDailyReport: taskTicket.isDailyReport,
  ...
  async updateOne(
    ...
    const updateItem: DeepPartial<TaskTicket> = {
      ...data,
      modifiedBy: user.id,
      modifiedDate: now,
    };
    ...
    return this.repository
      .createQueryBuilder(undefined, qr)
      .where('id = :id', { id: result.raw[0].id })
      .getOneOrFail();
  }
  ...
  // eslint-disable-next-line class-methods-use-this
  async getOneWithTx(
    params: FindOneOptions<TaskTicket>,
    queryRunner: QueryRunner,
  ): Promise<TaskTicket | null> {
    const data = await queryRunner.manager.findOne(TaskTicket, params);
