...
import { FarmingCycleOvkStockLog } from './FarmingCycleOvkStockLog.entity';
...
export class TaskTicket extends CMSBase {
  ...
  @OneToMany(() => FarmingCycleFeedStockD, (fcfs) => fcfs.taskTicket)
  @JoinColumn({ referencedColumnName: 'id' })
  feedStocks: FarmingCycleFeedStockD[];

  @OneToMany(() => FarmingCycleOvkStockLog, (fcfs) => fcfs.taskTicket)
  @JoinColumn({ referencedColumnName: 'id' })
  ovkStocks: FarmingCycleOvkStockLog[];

  @ManyToOne(() => FarmingCycleTaskD, (task) => task)
  @JoinColumn({ name: 'ref_farmingcycletask_id' })
  farmingCycleTask: FarmingCycleTaskD;

  @OneToOne(() => DailyMonitoring, (dm) => dm.taskTicket)
