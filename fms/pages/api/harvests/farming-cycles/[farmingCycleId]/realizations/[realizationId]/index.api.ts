...
export default async function handler(
  ...
  const farmingCycleId = req.query.farmingCycleId;
  const realizationId = req.query.realizationId;

  if (req.method === "DELETE") {
    await axios({
      method: "DELETE",
      url: `${process.env.BASE_URL_V2}/harvests/farming-cycles/${farmingCycleId}/realizations/${realizationId}`,
      headers: {
    ...
    if (
      !req.body.status ||
      !req.body.date ||
      req.body.bakulName === undefined ||
      !req.body.deliveryOrder ||
      !req.body.truckLicensePlate ||
      !req.body.driver ||
      !req.body.weighingNumber ||
      req.body.records === undefined
    ) {
      return res.status(400).json({
        code: 400,
        success: false,
