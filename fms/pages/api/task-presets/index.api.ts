...
export default async function handler(
  ...
  // NOTE: Get/search existing alert preset
  if (req.method === "GET") {
    if (!req.query) {
    ...
    const { page, limit, taskPresetCode, taskPresetName, coopTypeId, status } =
      req.query;

    await axios({
      method: "GET",
      url: `${process.env.BASE_URL_V2}/task-presets`,
      params: {
        $page: page,
        $limit: limit,
        taskPresetCode,
    ...
    if (
      !req.body.taskPresetCode ||
      !req.body.taskPresetName ||
      !req.body.coopTypeId ||
    ...
    await axios({
      ...
      url: `${process.env.BASE_URL_V2}/task-presets`,
      data: {
        taskPresetCode: req.body.taskPresetCode,
        taskPresetName: req.body.taskPresetName,
