...
export default async function handler(
  ...
  // NOTE: Get/search existing alert presets
  const ctId = req.query.id;
  if (req.method === "GET") {
    await axios({
      method: "GET",
      url: `${process.env.BASE_URL_V2}/alert-presets/${ctId}`,
      headers: {
    ...
    if (
      !req.body.alertPresetCode ||
      !req.body.alertPresetName ||
      !req.body.coopTypeId ||
      req.body.status === undefined ||
      req.body.alertIds === undefined
    ) {
      return res.status(400).json({
        code: 400,
        success: false,
    ...
    await axios({
      ...
      url: `${process.env.BASE_URL_V2}/alert-presets/${ctId}`,
      data: {
        alertPresetCode: req.body.alertPresetCode,
        alertPresetName: req.body.alertPresetName,
