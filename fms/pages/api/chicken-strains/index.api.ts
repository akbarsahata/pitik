...
export default async function handler(
  ...
  // NOTE: Get/search existing chicken strains
  if (req.method === "GET") {
    if (!req.query) {
    ...
    const { page, limit, status, chickTypeName } = req.query;

    await axios({
      method: "GET",
      url: `${process.env.BASE_URL_V2}/chick-types`,
      params: {
        $page: page,
        $limit: limit,
        status,
    ...
    if (
      !req.body.chickTypeCode ||
      !req.body.chickTypeName ||
    ...
    await axios({
      ...
      url: `${process.env.BASE_URL_V2}/chick-types`,
      data: {
        chickTypeCode: req.body.chickTypeCode,
        chickTypeName: req.body.chickTypeName,
