...
export default async function handler(
  ...
  // NOTE: Get/search existing cost plus contracts
  const contractId = req.query.id;
  if (req.method === "GET") {
    await axios({
      method: "GET",
      url: `${process.env.BASE_URL_V2}/contract/mitra-garansi/${contractId}`,
      headers: {
    ...
    if (
      ...
      !req.body.sapronak ||
      !req.body.chickenPrice ||
      !req.body.insentiveDeals ||
      !req.body.saving ||
      !req.body.deductionDueToFarmingCycleLoss
    ) {
      return res.status(400).json({
        code: 400,
        success: false,
