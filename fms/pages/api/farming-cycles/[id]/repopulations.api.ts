...
export default async function handler(
  ...
  // NOTE: Get/search existing farming cycle repopulations
  const farmingCycleId = req.query.id;
  if (req.method === "GET") {
    await axios({
      method: "GET",
      url: `${process.env.BASE_URL_V2}/farming-cycles/${farmingCycleId}/repopulations`,
      headers: {
    ...
    await axios({
      ...
      data: {
        ...
        repopulateDate: body.repopulateDate,
        repopulateReason: body.repopulateReason,
      },
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
