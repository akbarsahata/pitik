...
export default async function handler(
  ...
  // NOTE: Get/search existing target
  const ctId = req.query.id;
  if (req.method === "GET") {
    await axios({
      method: "GET",
      url: `${process.env.BASE_URL_V2}/targets/${ctId}`,
      headers: {
    ...
    if (
      ...
      !req.body.targetName ||
      !req.body.coopTypeId ||
      req.body.targetDaysCount === undefined ||
      !req.body.chickTypeId ||
      !req.body.variableId ||
    ...
    await axios({
      ...
      data: {
        ...
        targetDaysCount: req.body.targetDaysCount,
