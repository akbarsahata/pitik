...
import { postCreateCoopType } from "@services/api";
...
const Create: NextPage = () => {
  ...
  const createCoopType = useMutation(
    ["createCoopType"],
    async () =>
      await postCreateCoopType({
  ...
  if (createCoopType.isSuccess) return <Loading />;

  return (
    <MainWrapper headTitle="Create New Coop Type" pageTitle="New Coop Type">
          ...
          <div className="flex flex-1 w-full flex-col lg:flex-row items-start justify-start space-y-4 lg:space-y-0 space-x-0 lg:space-x-4">
            <div className="w-full">
              <Input
                ...
                errorMessage="Please set a valid coop name!"
      ...
      >
        <Button
          size="xs"
          state={createCoopType.isLoading ? "disabled" : "active"}
          type="outline"
          onClick={() => router.back()}
          title="Cancel"
        />
        <Button
          size="xs"
          state={createCoopType.isLoading ? "loading" : "active"}
          onClick={() => {
            const required = checkRequired({ dispatch, state });
            if (!required) return;

            createCoopType.mutate();
          }}
          title="Create Coop"
        />
      </div>
