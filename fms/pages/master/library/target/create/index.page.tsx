...
const Edit: NextPage = () => {
  ...
  const createTargetLibrary = useMutation(
    ["createTargetLibrary"],
    async () =>
      await postCreateTarget({
  ...
  if (
    createTargetLibrary.isSuccess ||
    coopTypeData.isLoading ||
    chickTypeData.isLoading ||
    variableData.isLoading
  )
    return <Loading />;
  if (coopTypeData.isError || chickTypeData.isError || variableData.isError)
    return <Error router={router} />;

  return (
    <MainWrapper
      headTitle="New Target Library"
      pageTitle={"New Target Library"}
    >
      <Modal
        footer={
      ...
      >
        <Button
          size="xs"
          state={createTargetLibrary.isLoading ? "disabled" : "active"}
          type="outline"
          onClick={() => router.back()}
          title="Cancel"
        />
        <Button
          size="xs"
          state={createTargetLibrary.isLoading ? "loading" : "active"}
          onClick={() => {
            const required = checkRequired({ dispatch, state });
            if (!required) return;

            createTargetLibrary.mutate();
          }}
          title="Create Target Library"
        />
      </div>
    </MainWrapper>
  );
