...
import {
  ...
  removeAllTriggers,
  setErrorText,
  setTaskLibraryInitialData,
} from "./edit.functions";
import InstructionsModal from "./edit.instructions";
import { reducer } from "./edit.reducer";

const Edit: NextPage = () => {
  const router = useRouter();
  const taskLibraryId = decodeString(router.query.id as string);
  ...
  const getThisTaskLibrary: UseQueryResult<
    { data: TGetByIdResponse<TTaskLibraryResponse> },
    AxiosError
  > = useQuery(
    ["getThisTaskLibrary"],
    async () => await getTaskLibrary(taskLibraryId),
    {
      refetchOnWindowFocus: false,
      onError: (error) => {
        console.log(error.response?.data);
  ...
  const editTaskLibrary = useMutation(
    ["putTaskLibrary"],
    async () => {
  ...
  if (
    getThisTaskLibrary.isLoading ||
    getThisTaskLibrary.isFetching ||
    variableData.isLoading ||
    feedbrandsData.isLoading ||
    editTaskLibrary.isLoading ||
    editTaskLibrary.isSuccess
  )
    return <Loading />;
  if (
    variableData.isError ||
    feedbrandsData.isError ||
    getThisTaskLibrary.isError
  )
    return <Error router={router} />;

  return (
    <MainWrapper
      headTitle="Edit Task Library"
      pageTitle={`Edit Task Library - ${state.oldName}`}
    >
      <Modal
        onCancel={() => {
          dispatch({
      ...
      >
        ...
        <Button
          ...
          onClick={() => {
            ...
            editTaskLibrary.mutate();
          }}
          title="Edit Task Library"
        />
      </div>
    </MainWrapper>
  );
