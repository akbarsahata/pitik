...
import { getRoles, getUsers, postCreateUser } from "@services/api";
...
const Create: NextPage = () => {
  ...
  let userTypeOptions: IDropdownItem<TRoleResponse>[] = [];
  state.roleData.map((item: TRoleResponse) =>
  ...
  > = useQuery(["roleData"], async () => await getRoles({ limit: 50 }), {
    ...
    onSuccess: ({ data }) => {
      ...
      const poultry = roleDataList.filter(
        (data) =>
          data.name === USER_TYPE.KK.full || data.name === USER_TYPE.AK.full
      );
  ...
  const createPoultry = useMutation(
    ["userData"],
    async () =>
      await postCreateUser({
        userType: state.userType?.value.name.toLowerCase() || "",
  ...
  if (
    ownerData.isLoading ||
    createPoultry.isSuccess ||
    createPoultry.isLoading ||
    ownerData.isLoading
  )
    return <Loading />;
  ...
  return (
    <MainWrapper
      headTitle="Create New Poultry User"
      pageTitle="New Poultry User Registration"
    >
          ...
          <div className="flex flex-1 w-full flex-col lg:flex-row items-start justify-start space-y-4 lg:space-y-0 space-x-0 lg:space-x-4">
            ...
            <div className="flex-1 w-full">
              <Input
                ...
                errorMessage="Please set a valid poultry code!"
      ...
      >
        <Button
          size="xs"
          state={createPoultry.isLoading ? "disabled" : "active"}
          type="outline"
          onClick={() => router.back()}
          title="Cancel"
        />
        <Button
          size="xs"
          state={createPoultry.isLoading ? "loading" : "active"}
          onClick={() => {
            const required = checkRequired({ dispatch, state });
            if (!required) return;
            createPoultry.mutate();
          }}
          title="Create Poultry"
        />
      </div>
    </MainWrapper>
  );
