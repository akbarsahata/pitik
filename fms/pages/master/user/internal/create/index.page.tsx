...
import { getParents, getRoles, postCreateUser } from "@services/api";
...
const Create: NextPage = () => {
  ...
  if (createUser.isSuccess || createUser.isLoading || roleData.isLoading)
    return <Loading />;
  if (roleData.isError || parentData.isError) return <Error router={router} />;

  return (
    <MainWrapper
      headTitle="Create New Internal User"
      pageTitle="New Internal User Registration"
    >
      <div className="grid grid-cols-1 md:grid-cols-2 mt-10 space-y-6 md:space-y-0 md:space-x-8">
        <div className="flex flex-col items-start justify-start space-y-6">
          <div className="flex flex-1 w-full flex-col lg:flex-row items-start justify-start space-y-4 lg:space-y-0 space-x-0 lg:space-x-4">
            <div className="w-full">
              <Dropdown
                ...
                errorMessage="Please select an upstream role!"
          ...
          <div className="flex flex-1 w-full flex-col lg:flex-row items-start justify-start space-y-4 lg:space-y-0 space-x-0 lg:space-x-4">
            <div className="w-full">
              <Dropdown
                label="Downstream Role"
                state={
                  state.errorType === ERROR_TYPE.USER_TYPE ? "error" : "active"
                }
                isSearchable={true}
                isLoading={roleData.isLoading ? true : false}
                errorMessage="Please select a downstream role!"
                value={state.downstreamRole}
                options={downstreamRoleOptions}
                onChange={(item: IDropdownItem<TRoleResponse>) => {
                  // dispatch({
                  //   type: ACTION_TYPE.SET_PARENT,
                  //   payload: null,
