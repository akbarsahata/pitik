...
import {
  checkRequired,
  setErrorText,
  setTaskPresetInitialData,
} from "./edit.functions";
import TaskModal from "./edit.modal";
import { reducer } from "./edit.reducer";

const Edit: NextPage = () => {
  const router = useRouter();
  const [state, dispatch] = useReducer(reducer, store);
  const taskPresetId = decodeString(router.query.id as string);
  ...
  const taskPresetData: UseQueryResult<
    { data: TGetByIdResponse<TTaskPresetResponse> },
    AxiosError
  > = useQuery(
    ["taskPresetData"],
    async () => await getTaskPreset(taskPresetId),
    {
  ...
  const editTaskPreset = useMutation(
    ["editTaskPreset"],
    async () =>
      await putEditTaskPreset(
        {
  ...
  if (
    taskPresetData.isFetching ||
    editTaskPreset.isSuccess ||
    coopTypeData.isLoading ||
    taskPresetData.isLoading
  )
    return <Loading />;
  if (coopTypeData.isError || taskData.isError || taskPresetData.isError)
    return <Error router={router} />;

  return (
    <MainWrapper
      headTitle="Edit Task Preset"
      pageTitle={"Edit Task Preset - " + state.oldName}
    >
      ...
      >
        <Button
          size="xs"
          state={taskPresetData.isLoading ? "disabled" : "active"}
          type="outline"
          onClick={() => router.back()}
          title="Cancel"
        />
        <Button
          size="xs"
          state={taskPresetData.isLoading ? "loading" : "active"}
          onClick={() => {
            const required = checkRequired({ dispatch, state });
            if (!required) return;

            editTaskPreset.mutate();
          }}
          title="Edit Task Preset"
        />
      </div>
    </MainWrapper>
  );
