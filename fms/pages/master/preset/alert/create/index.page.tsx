...
import { getAlerts, getCoopTypes, postCreateAlertPreset } from "@services/api";
...
import AlertModal from "./create.modal";
...
const Create: NextPage = () => {
  ...
  const createAlertPreset = useMutation(
    ["createAlertPreset"],
    async () =>
      await postCreateAlertPreset({
  ...
  if (
    createAlertPreset.isLoading ||
    createAlertPreset.isSuccess ||
    coopTypeData.isLoading
  )
    return <Loading />;
  if (coopTypeData.isError || alertData.isError)
    return <Error router={router} />;

  return (
    <MainWrapper
      headTitle="Create New Alert Preset"
      pageTitle="New Alert Preset"
    >
      ...
      >
        <Button
          size="xs"
          state={createAlertPreset.isLoading ? "disabled" : "active"}
          type="outline"
          onClick={() => router.back()}
          title="Cancel"
        />
        <Button
          size="xs"
          state={createAlertPreset.isLoading ? "loading" : "active"}
          onClick={() => {
            const required = checkRequired({ dispatch, state });
            if (!required) return;

            createAlertPreset.mutate();
          }}
          title="Create Alert Preset"
        />
      </div>
    </MainWrapper>
  );
