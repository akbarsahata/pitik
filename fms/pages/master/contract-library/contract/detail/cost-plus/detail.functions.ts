...
export const setCostPlusInitialData = ({
  dispatch,
  data,
  state,
}: {
  ...
  let marginCostPlusBop: TMarginCostPlusBop[] = [];
  data.bop.map((item) => {
    marginCostPlusBop.push({
      id: item.id,
      preConditions: parseInt(item.preConditions),
      bop: item.bop,
      amount: item.amount,
      paymentTerm: {
  ...
  dispatch({
    ...
    payload: data.contractDeductionFcBop,
  });
  dispatch({
...
export const checkRequired = ({
  ...
  if (
    ...
    isNaN(getMarginCostBopValues("3", state).preConditions)
  ) {
    dispatch({
      type: ACTION_TYPE.SET_ERROR_TYPE,
...
export const getMarginCostBopValues = (
  ...
  return state.marginCostPlusBop[
    state.marginCostPlusBop.findIndex(
      (item: TMarginCostPlusBop) => item.bop === id
    )
  ];
};
...
export const getDeductionFcLossValues = (
  ...
  return state.contractDeductionFcBop[
    state.contractDeductionFcBop.findIndex((item) => item.bop === id)
  ];
};
