...
const Edit: NextPage = () => {
  ...
  const buildingId = decodeString(router.query.id as string);
  ...
  const buildingData: UseQueryResult<
    { data: TGetByIdResponse<TBuildingResponse> },
    AxiosError
  > = useQuery(["buildingData"], async () => await getBuilding(buildingId), {
    refetchOnWindowFocus: false,
    enabled: !!state.buildingTypeData.length && !!state.farmData.length,
    onError: (error) => {
      console.log(error.response?.data);
    },
    onSuccess: ({ data }) => {
  ...
  if (
    editBuilding.isLoading ||
    editBuilding.isSuccess ||
    farmData.isLoading ||
    buildingTypeData.isLoading ||
    farmData.isFetching ||
    buildingTypeData.isFetching ||
    buildingData.isLoading ||
    buildingData.isFetching
  )
    return <Loading />;
  if (buildingTypeData.isError || farmData.isError || buildingData.isError)
    return <Error router={router} />;

  return (
    <MainWrapper
      headTitle="Edit Building"
      pageTitle={`Edit Building - ${state.oldName}`}
    >
      ...
      >
        <Button
          size="xs"
          state={editBuilding.isLoading ? "disabled" : "active"}
          type="outline"
          onClick={() => router.back()}
          title="Cancel"
        />
        <Button
          size="xs"
          state={editBuilding.isLoading ? "loading" : "active"}
          onClick={() => {
