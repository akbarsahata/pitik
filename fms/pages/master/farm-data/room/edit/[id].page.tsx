...
import FanModal from "./edit.fan";
import {
  checkRequired,
  setErrorText,
  setRoomInitialData,
} from "./edit.functions";
import HeaterModal from "./edit.heater";
import { reducer } from "./edit.reducer";

const Create: NextPage = () => {
  const router = useRouter();
  const [state, dispatch] = useReducer(reducer, store);
  const roomId = decodeString(router.query.id as string);
  ...
  const roomData: UseQueryResult<
    { data: TGetByIdResponse<TRoomResponse> },
    AxiosError
  > = useQuery(["roomData"], async () => await getRoom(roomId), {
    enabled:
      !!state.coopData.length &&
      !!state.buildingData.length &&
      !!state.roomTypeData.length &&
      !!state.floorTypeData.length &&
      !!state.controllerTypeData.length &&
      !!state.heaterTypeData.length,
    refetchOnWindowFocus: false,
    onError: (error) => {
      console.log(error.response?.data);
    },
      ...
      state.heaterInRooms.length &&
        state.heaterInRooms.map((item: THeaterInRoomsBody) =>
          heaterPayload.push({
            ...
            heaterTypeId: item.heaterType?.value.id || "",
          })
        );
  ...
  if (
    ...
    roomTypeData.isFetching ||
    floorTypeData.isFetching ||
    controllerTypeData.isFetching ||
    heaterTypeData.isFetching ||
    roomData.isFetching ||
    editRoom.isLoading ||
    editRoom.isSuccess
  )
  ...
  return (
    <MainWrapper
      headTitle="Edit Room"
      pageTitle={`Edit Room - ${state.oldName}`}
    >
      ...
      >
        <Button
          size="xs"
          state={editRoom.isLoading ? "disabled" : "active"}
          type="outline"
          onClick={() => router.back()}
          title="Cancel"
        />
        <Button
          size="xs"
          state={editRoom.isLoading ? "loading" : "active"}
          onClick={() => {
