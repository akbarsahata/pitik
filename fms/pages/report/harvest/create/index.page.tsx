...
import DetailsAccordion from "./create.accordion";
import {
  ACTIONS,
  ERROR_TYPE,
  initialState,
...
const Create: NextPage = () => {
  ...
  > = useQuery(
    ...
    async () =>
      await getCoops({
        ...
        limit: 0,
        status: true,
        farmingCycleStatus: "IN_PROGRESS",
      }),
    {
  ...
  const farmingCycleData: UseQueryResult<
    ...
    AxiosError
  > = useQuery(
    ["farmingCycleData", state.coop],
    async () =>
      await getFarmingCycles({
        page: 1,
        limit: 1,
        farmingStatus: "2",
        coopId: state.coop.value.id,
      }),
    {
      refetchOnWindowFocus: false,
      enabled: !!state.coop,
      ...
      onSuccess: ({ data }) => {
        const farmingCycleId = data.data[0]?.farmingCycleId;
        dispatch({
          type: ACTIONS.SET_FARMING_CYCLE_ID,
          payload: { data: farmingCycleId },
        });
        detailsData.mutate(farmingCycleId);
      },
    }
  );

  const detailsData: any = useMutation(
    ["detailsData"],
    async (farmingCycleId: string) =>
      await getFarmingCycleHarvestDetail(farmingCycleId),
    {
      onError: (error: any) => {
        console.log(error.response?.data);
      },
  ...
  const createHarvestRealization: any = useMutation(
    ["createHarvestRealization"],
    async (status: "DRAFT" | "FINAL" | "DELETED") => {
      let dataTimbangData: TWeighingDataPayload[] = [];
      state.records.map((dataTimbang: TWeighingDataPayload) =>
        dataTimbangData.push({
          quantity: dataTimbang.quantity,
          tonnage: dataTimbang.tonnage,
        })
      );
      await postCreateRealization(
        {
          status: status,
          date: state.date,
          ...
          records: dataTimbangData,
        },
        state.farmingCycleId
      );
    },
    {
      onError: (error: any) => {
  ...
  if (
    createHarvestRealization.isLoading ||
    createHarvestRealization.isSuccess ||
    coopData.isLoading ||
    farmingCycleData.isLoading ||
    detailsData.isLoading
  )
    return <Loading />;
  if (coopData.isError || farmingCycleData.isError)
    return <Error router={router} />;
  return (
    <MainWrapper
      headTitle="Add Harvest Realization"
      pageTitle="Add Harvest Realization"
    >
      <Modal
        content={
        ...
        footer={
          <div className="flex flex-1 space-x-2 items-center justify-end">
            <Button
              ...
              title="Cancel"
              size="xs"
              state={createHarvestRealization.isLoading ? "loading" : "active"}
              onClick={() => {
                dispatch({
            ...
            <Button
              ...
              onClick={() => {
                ...
                createHarvestRealization.mutate(
                  state.saveAsFinal
      ...
      />
      {state.coop !== null && state.farmingCycleId !== undefined && (
        <DetailsAccordion data={state.detailHarvestData} />
      )}
            ...
            <Input
              ...
              state={
                state.errorType === ERROR_TYPE.DATE
                  ...
                  : state.coop === null
                  ? "disabled"
                  : state.farmingCycleId === undefined
                  ? "disabled"
                  : "active"
              }
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-0">
              <p className="text-gray-400">
                Please enter the weight data for each row based on the
                information provided in the Kartu Timbang (weighing card)
              </p>
              <p className="italic text-gray-500">
                Note that if the weight data is in decimal form, use a dot (.)
                    ...
                    >
                      <div className="w-12 flex items-start justify-center">
                        <Button
                          ...
                          state={state.records ? "active" : "disabled"}
                        ...
                        <Input
                          ...
                          onKeyDown={(event) => {
                            ...
                            if (event.key === "Enter") {
                              ...
                              setTimeout(() => {
                                const nextTonnageIndex =
                                  currentTonnageIndex + 1;
      ...
      {state.coop === null ||
        (state.farmingCycleId === undefined && (
          <div
            className={`flex bg-red-100 px-4 py-4 flex-row items-center justify-start rounded mt-4`}
          >
            <div className="mr-2 text-red-500 text-xl">
              <WarningIcon />
            </div>
            <p className="text-red-500">
              {"This coop doesn't have an active farming cycle"}
            </p>
          </div>
        ))}
      <div
      ...
      >
        <Button
          size="xs"
          state={createHarvestRealization.isLoading ? "disabled" : "active"}
          type="outline"
          onClick={() => {
            dispatch({
              type: ACTIONS.SET_WEIGHING_DATA,
