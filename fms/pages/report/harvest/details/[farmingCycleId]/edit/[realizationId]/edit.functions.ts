...
import { THarvestRealizationDetailResponse } from "@type/response.type";
import {
  ACTIONS,
  ERROR_TYPE,
  TState,
...
export const setHarvestRealizationInitialData = ({
  dispatch,
  data,
}: {
  dispatch: any;
  data: THarvestRealizationDetailResponse;
  state: TState;
}) => {
  dispatch({
  ...
  dispatch({
    ...
    payload: { data: data.bakulName },
  });

  dispatch({
    type: ACTIONS.SET_DELIVERY_ORDER,
    payload: { data: data.deliveryOrder },
  });
  ...
  dispatch({
    ...
    payload: { data: data.weighingNumber },
  });

  dispatch({
    type: ACTIONS.SET_TRUCK_LICENSE_PLATE,
    payload: { data: data.truckLicensePlate },
  });
