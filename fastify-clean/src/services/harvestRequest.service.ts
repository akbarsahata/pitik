...
import { FARMING_ACTIVITIES } from '../libs/constants';
...
export class HarvestRequestService {
    ...
    try {
      ...
      return harvestRequests.map((hr) => {
        const status = mapActivityStatusBasedRoleRank(
          ...
          FARMING_ACTIVITIES.HARVEST_REQUEST,
        );
  ...
  async approveHarvestRequest(
    ...
    let approvedHR = await this.dao.getOneStrict({
      where: {
        id: params.harvestRequestId,
      },
    });
    ...
    try {
      approvedHR = await this.dao.updateOneWithTx(
        {
          id: params.harvestRequestId,
        },
        {
      ...
      await this.harvestRequestApprovedQueue.addJob(approvedHR);
