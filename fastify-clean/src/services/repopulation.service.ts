...
export class RepopulationService {
  ...
  async create(
    ...
    try {
      ...
      // FIXME: please remove import env once flow with ODOO is ready
      if (env.USE_ERP) {
        this.erpDAO.createRepopulation(farmingCycle.farmingCycleCode, repopulationData);
      }

      await this.repopulationCreatedQueue.addJob(repopulationData);
      ...
      if (defaultTaskTicket) {
        await this.calculateDailyMonitoringQueue.addJob({
          farmingCycleId,
          taskTicketId: defaultTaskTicket.taskTicketId,
        });
      }
