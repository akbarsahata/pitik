...
import { GamificationTaskSubmittedQueue } from '../jobs/queues/gamification-task-submitted.queue';
...
import { DEFAULT_TIME_ZONE, FC_CYCLE_STATUS, FC_FARMING_STATUS } from '../libs/constants';
...
export class TaskService {
  ...
  @Inject(GamificationTaskSubmittedQueue)
  private taskSubmittedQueue: GamificationTaskSubmittedQueue;
  ...
  async getTask(filter: GetTaskQuery): Promise<[Task[], number]> {
    ...
    return this.taskDAO.getMany({
      where: {
        ...
        taskName: filter.taskName ? Like(`%${filter.taskName}%`) : undefined,
  ...
  async getTaskDetail(taskId: string): Promise<TaskDetail> {
    return this.dao.getOneDetailById(taskId);
  }
  ...
  async generateLateTaskReminderJobs() {
    ...
    activeFarmingCycles.reduce(async (prev, fc) => {
      await prev;
