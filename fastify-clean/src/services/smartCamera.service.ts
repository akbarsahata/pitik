...
export class SmartCameraService {
  ...
  async getDayRecords(opts: {
    ...
    const isInternal =
      opts.user.roleRank &&
      opts.user.roleRank.internal &&
      parseInt(opts.user.roleRank.internal, 10) >= 0;
    const [jobs] = await this.smartCameraJobDAO.getMany({
      where: {
        sensorId: In(sensorIds),
        createdAt: Raw(
          (createdDate) => `${createdDate} <= NOW() AND ${createdDate} >= :farmingCycleStartDate`,
  ...
  async getRecordImages(opts: {
    ...
    const [records, count] = await this.smartCameraJobDAO.getMany({
      where: {
        sensorId: In(sensorIds),
        createdAt: Raw(
          (createdAt) => `${createdAt} >= :min::timestamp AND ${createdAt} <= :max::timestamp`,
          dateFilter,
        ),
        requestedBy: (!isInternal && Not(IsNull())) || undefined,
        crowdResults:
    ...
    await Promise.all(
      records.map(async (record) => {
        const resultData = {
          ...
          temperature: null,
          humidity: null,
          link: `${env.AWS_AI_URL}${env.SMART_CAMERA_FOLDER}/chicken-behavior/coop_${record.coopId}/room_${record.floorId}/cam_${record.sensorId}/${record.imagePath}`,
  ...
  async createJob(opts: {
    ...
    const now = format(new Date(), SMART_CAMERA_DATE_TIME_FORMAT);
    ...
    try {
      job = await this.smartCameraJobDAO.upsertOne({
        ...
        item: {
          id: jobId,
          bucket: opts.user ? env.AWS_AI_PUBLIC_BUCKET : env.AWS_AI_BUCKET,
          sensorId: sensor.id,
          coopId,
      ...
      this.smartCameraJobDAO.rollbackTransaction(qr);
    ...
    if (!opts.user) {
      try {
        const command = new PutObjectCommand({
          Bucket: job.bucket,
          Key: pathFile,
          ACL: job.bucket === env.AWS_AI_PUBLIC_BUCKET ? 'public-read' : undefined,
        });

        presignedUrl = await getSignedUrl(this.s3.client, command, {
          expiresIn: 15 * 60,
        });
        job = await this.smartCameraJobDAO.upsertOne({
          qr,
    ...
    await Promise.all(
      results.map(async (result, idx, arr) => {
        const resultData = {
          ...
          temperature: null,
          humidity: null,
          link: `${env.AWS_AI_URL}${result.createJobItem.pathFile}`,
