...
export class SensorService {
  ...
  async getCoopSensorLatestCondition(
    ...
    if (query.roomId) {
      const { sensors, idDevice } = await this.getSensorListWithinRoom(query.roomId);
      sensorList = sensors;
      deviceId = idDevice;
    }
    ...
    const sensorData = await [1, 3, 5, 7].reduce(
      async (prevSensorData, hours): Promise<SearchHit<IOTSensorData>[]> => {
        ...
        const result = await this.sensorESDAO.getManyByCoopCode(
          coopCode,
          {
            from: sub(now, { hours }),
            to: now,
  ...
  async getCoopSensorHistorical(
    ...
    let deviceId: string | null = null;
    if (roomId) {
      const { sensors, idDevice } = await this.getSensorListWithinRoom(roomId);
      sensorList = sensors;
      deviceId = idDevice;
    }
    ...
    const sensorDataCurrent = await this.sensorESDAO.getManyByCoopCode(
      coopCode,
      currentTimeRange,
      'asc',
      deviceId,
  ...
  private async getSensorListWithinRoom(roomId: string) {
    ...
    if (roomDetail) {
      roomDetail?.iotSensors.forEach((sensor: Partial<IotSensor>) => {
        ...
        idDevice = sensor.iotDevice?.deviceId || null;
      });
    }
