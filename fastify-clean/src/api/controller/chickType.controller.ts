...
import { ChickTypeService } from '../../services/chickType.service';
...
export class ChickTypeController {
  ...
  private service: ChickTypeService;
  ...
  @POST({
    ...
    options: {
      schema: {
        ...
        response: {
          201: createChickTypeResponseDTO,
        },
      },
  ...
  async create(request: createChickTypeRequest, reply: FastifyReply) {
    const chickType = await this.service.create(request.body, request.user);
  ...
  async get(request: getChickTypesRequest): Promise<GetChickTypesResponse> {
    const [chickTypes, count] = await this.service.get(request.query);
  ...
  async getById(request: getChickTypeByIdRequest): Promise<GetChickTypeByIdResponse> {
    const chickType = await this.service.getById(request.params.id);
  ...
  async update(request: updateChickTypeByIdRequest): Promise<UpdateChickTypeByIdResponnse> {
    const updatedChickType = await this.service.update(
      request.params.id,
