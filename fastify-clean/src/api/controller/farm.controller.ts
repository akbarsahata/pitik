...
export class FarmController {
  ...
  async getActiveFarmingCycles(request: ActiveFarmingCycleRequest) {
    const activeFarmingCycles = await this.service.getActiveFarmingCycle(request.user);
  ...
  async create(request: CreateFarmRequest): Promise<CreateFarmResponse> {
    const farm = await this.service.create(request.body, request.user);
    return {
      code: 201,
      data: farm,
    };
  ...
  async getFarms(request: GetFarmsRequest): Promise<GetFarmsResponse> {
    const [farms, count] = await this.service.getFarms(request.query);
    return {
      code: 200,
      count,
      data: farms.map((farm) => ({
  ...
  async getFarmById(request: GetFarmByIdRequest): Promise<GetFarmByIdResponse> {
    const farm = await this.service.getFarmById(request.params.id);
    return {
      code: 200,
      data: {
        ...farm,
  ...
  async updateFarm(request: UpdateFarmRequest): Promise<UpdateFarmResponse> {
    const farm = await this.service.update(
      request.params.id,
      {
        ...request.body,
