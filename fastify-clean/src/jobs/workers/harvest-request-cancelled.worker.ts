...
@Service()
export class HarvestRequestCancelledWorker extends BaseWorker<HarvestRequest> {
  ...
  protected workerName = QUEUE_HARVEST_REQUEST_CANCELLED;
  ...
  protected async handle(harvestRequest: HarvestRequest): Promise<void> {
    try {
      ...
      const userTypes = [...USER_TYPE_REQUESTER_GROUP, ...USER_TYPE_APPROVER_GROUP];
      const headline =
        `[INFO] Pengajuan panen kandang ${farmingCycle.coop.coopName} ` +
        `${format(new Date(harvestRequest.datePlanned), 'd MMM')} dibatalkan`;
