...
import { USER_TYPE_REQUESTER_GROUP } from '../../libs/constants';
import { QUEUE_HARVEST_REQUEST_EDITED } from '../../libs/constants/queue';
...
@Service()
export class HarvestRequestEditedWorker extends BaseWorker<HarvestRequest> {
  ...
  protected workerName = QUEUE_HARVEST_REQUEST_EDITED;
  ...
  protected async handle(harvestRequest: HarvestRequest): Promise<void> {
    try {
      ...
      const headline =
        `[INFO] Perubahan pengajuan panen kandang ${farmingCycle.coop.coopName} ` +
        `${format(new Date(harvestRequest.datePlanned), 'd MMM')}`;

      const [userReceivers] = await this.coopMemberDDAO.getMany({
        where: {
          coopId: farmingCycle.coopId,
          user: {
            userType: In(USER_TYPE_REQUESTER_GROUP),
          },
        },
        relations: {
          user: true,
